...
use Vectorface\PhpCryptocurrencyAddressValidation\Utils\Sha3;
use Vectorface\PhpCryptocurrencyAddressValidation\Validation;

class ETH extends Validation
{
    public function isAddress(string $address, $options = []): bool
    {
        ...
        if (!preg_match('/^(0x)[0-9a-f]{40}$/i', $address)) {
            ...
            return $this->isChecksumAddress($address, $options);
        }
    }

    public function isChecksumAddress(string $address, array $options = []): bool
    {
        // Check each case
    ...
    {
        return $this->isAddress($address, $options);
    }
}
